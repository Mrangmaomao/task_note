...
js编程本来是单线程执行，接口请求，如果失败之后都会导致页面阻塞，JS异步编程就是用来解决这个问题的，可以得到页面性能优化提升。
EventLoop、消息队列，宏任务，微任务 都是支持js异步工作的主要内容
在js引擎执行一段代码的过程中，主要会经历以下几个阶段
1. 读取代码 
2. 压入执行栈 
3. 执行栈处理各个任务 
4. 遇到同步任务，处理完，排出执行栈
5. 遇到异步任务，执行栈处理完后，会委托宿主环境区执行任务，约定执行后，再将与这个任务的执行结构通过之前约定好的回调函数注册到消息队列
6. 通过eventloop检测，将消息队列中的任务压入执行栈，再依次执行
宏任务是消息队列里的任务，常见的接口请求，定时器等异步任务都是宏任务
微任务是异步任务，不需要通过eventloop监测，通过消息队列取出并压入执行栈中再执行，像promise是微任务
...
```
    ...
    fp.flowRight(_average,fp.map(getDollarValue));
```
...
fp.flowRight(_underscore,toLowerCase,getDollarName)
```
